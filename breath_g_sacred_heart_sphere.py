...
st.title("Breath G • The Living Sacred Heart Sphere — v2")
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Prototype 02 • Streamlit + Three.js")
...
<body>
<div id="stage">
  <div id="loadmsg" style="position:absolute;left:50%;top:52%;transform:translate(-50%,-50%);z-index:3;color:#fff0c5;font:18px Georgia,serif;text-align:center;opacity:.9;">
    Awakening the sphere...
  </div>
  <div id="hud">
...
if (typeof THREE === "undefined") {
  document.getElementById("loadmsg").innerHTML =
    "Three.js did not load.<br>Please refresh once, or check that the browser is not blocking cdn.jsdelivr.net.";
...
centerHalo.scale.set(3.2,3.2,1);
...
const sphereMat = new THREE.MeshPhysicalMaterial({
  color:0x5c4a9f, transparent:true, opacity:.18, roughness:.2, metalness:.05,
  transmission:.45, thickness:.35, side:THREE.DoubleSide,
  emissive:c3, emissiveIntensity:.18
});
...
const wire = new THREE.Mesh(
  ...
  new THREE.MeshBasicMaterial({color:c1,wireframe:true,transparent:true,opacity:.18,blending:THREE.AdditiveBlending})
);
group.add(wire);
...
const memories=new THREE.Points(memGeo,new THREE.PointsMaterial({
  size:.065,vertexColors:true,transparent:true,opacity:.95,blending:THREE.AdditiveBlending,depthWrite:false
}));
group.add(memories);
